...
class HumanFindDoctorsPage extends StatefulWidget {
  final Function() onBookAppointment;
  
  const HumanFindDoctorsPage({
    Key? key,
    required this.onBookAppointment,
  }) : super(key: key);

  @override
  State<HumanFindDoctorsPage> createState() => _HumanFindDoctorsPageState();
}

class _HumanFindDoctorsPageState extends State<HumanFindDoctorsPage> {
  ...
  Future<List<DoctorsDb>> _fetchDoctors() async {
    if (_cachedDoctors != null) {
      debugPrint('📦 [FindDoctors] Using cached doctors list');
      return _cachedDoctors!;
    }

    setState(() => _isLoading = true);
    try {
      debugPrint('🔍 [FindDoctors] Starting to fetch doctors...');
      final querySnapshot = await FirebaseFirestore.instance
          .collection('doctors')
          .where('doctorType', isEqualTo: 'Human')
          .get();

      debugPrint('📊 [FindDoctors] Query snapshot size: ${querySnapshot.docs.length}');
      
      if (querySnapshot.docs.isEmpty) {
        debugPrint('⚠️ [FindDoctors] No doctors found in the collection');
        return [];
      }
      ...
      final doctors = querySnapshot.docs.map((doc) {
        try {
          final data = doc.data();
          debugPrint('👨‍⚕️ [FindDoctors] Processing doctor: ${data['name']}');
          debugPrint('📝 [FindDoctors] Doctor data: ${data.toString()}');
          return DoctorsDb.fromJson(data);
        } catch (e) {
          debugPrint('❌ [FindDoctors] Error processing doctor document: $e');
          return null;
        }
      }).whereType<DoctorsDb>().toList();

      debugPrint('✅ [FindDoctors] Successfully fetched ${doctors.length} doctors');
      _cachedDoctors = doctors;
      return doctors;
    } catch (e) {
      debugPrint('❌ [FindDoctors] Error fetching doctors: $e');
      return [];
    } finally {
  ...
  List<String> _getSpecializations(List<DoctorsDb> doctors) {
    ...
    debugPrint('🏷️ [FindDoctors] Available specializations: $specializations');
  ...
  List<DoctorsDb> _filterDoctors(List<DoctorsDb> doctors) {
    ...
    debugPrint('🔍 [FindDoctors] Filtered doctors count: ${filtered.length}');
    return filtered;
  }
                      ...
                      FutureBuilder<List<DoctorsDb>>(
                        ...
                        builder: (context, snapshot) {
                          ...
                          if (snapshot.hasError) {
                            debugPrint('❌ [FindDoctors] Error in specialization dropdown: ${snapshot.error}');
                            return Container(
                              width: 200,
                              padding: const EdgeInsets.symmetric(horizontal: 16),
                              decoration: BoxDecoration(
            ...
            if (_isLoading)
              ...
              FutureBuilder<List<DoctorsDb>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasError) {
                    debugPrint('❌ [FindDoctors] Error in doctors list: ${snapshot.error}');
                    return Center(
                      child: Column(
  ...
  Widget _buildDoctorCard(DoctorsDb doctor) {
    return Card(
